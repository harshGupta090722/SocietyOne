          ...
          <tbody className="bg-white divide-y divide-slate-200 text-sm">
            {filteredPayments.length > 0 ? (
              filteredPayments.map((payment) => {
                const fileUrl = getUploadUrl(payment.screenshotURL || '');
                ...
                return (
                  <tr key={payment.paymentId} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${payment.status === 'approved'
