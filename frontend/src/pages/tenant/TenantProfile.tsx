...
function TenantProfile() {
  ...
  return (
    <div className="max-w-3xl mx-auto font-sans p-2 bg-[#f8fafc] min-h-[calc(100vh-120px)] pb-12">
      ...
      {(() => {
        // Current submission first, then logged previous attempts (most recent first)
        const attempts: VerificationAttempt[] = verification?.attempts
          ? [...verification.attempts].reverse()
          : [];
        const history: VerificationAttempt[] = verification
          ? [
              {
        ...
        return (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 space-y-4 mt-8 animate-fadeIn">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Review the status, reason, and date of all your verification submissions.
                </p>
              </div>
              <button
                type="button"
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-blue-600 bg-blue-50 border border-blue-100 rounded-xl hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
                {showHistory ? (
                  <>
                  ...
                  <>
                    Show Previous Requests <ChevronDown className="w-4 h-4" />
                  </>
                )}
              </button>
            </div>
            ...
            {history.length === 0 ? (
              <div className="border-t border-slate-100 pt-6 text-center text-sm text-slate-400 font-medium">
                You haven't submitted any verification requests yet.
              </div>
            ) : (
              showHistory && (
                <div className="border-t border-slate-100 pt-6 space-y-3 animate-fadeIn">
                  {history.map((req, idx) => (
                    <div
                      key={idx}
                    ...
                    >
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          <span
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusStyles(req.status)}`}
                          >
                      ...
                      {req.idProofUrl && (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-800 transition-colors flex-shrink-0"
                        >
